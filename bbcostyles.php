...
if ($slashargument = min_get_slash_argument()) {
    ...
    $subthemeid = min_optional_param('subtheme', '1', 'INT');
}

$subtheme = empty($subthemeid) ? '' : '_' . $subthemeid;
...
$candidatesheet = "{$candidatedir}/" . theme_styles_get_filename($type, $themesubrev, $usesvg, $subtheme);
...
$subthemeobj = utils::get_subtheme($subthemeid, false);
utils::set_subtheme($subthemeobj);
...
if ($sendaftergeneration || $lock) {
    ...
    $candidatesheet = theme_styles_generate_and_store($theme, $rev, $themesubrev, $candidatedir, $subtheme);
 ...
 * @param   string          $subtheme The subtheme to be used
 * @return  string          The path that the primary CSS was written to
 */
function theme_styles_generate_and_store($theme, $rev, $themesubrev, $candidatedir, $subtheme) {
    global $CFG;
    require_once("{$CFG->libdir}/filelib.php");

    // Generate the content first.
    if (!$csscontent = bambuco_get_css_cached_content($theme, $subtheme)) {
        $csscontent = $theme->get_css_content();
        bambuco_set_css_content_cache($theme, $subtheme, $csscontent);
    }
    ...
    $candidatesheet = "{$candidatedir}/" . theme_styles_get_filename($type, $themesubrev, $theme->use_svg_icons(), $subtheme);
    ...
    $fallbacksheet = make_temp_directory("theme/{$theme->name}")
        . "/"
        . theme_styles_get_filename($type, 0, $theme->use_svg_icons(), $subtheme);
 ...
 */
function theme_styles_get_filename($type, $themesubrev = 0, $usesvg = true, $subtheme = '') {
    $filename = $type;
...
function bambuco_get_css_cached_content($theme, $subtheme) {
    ...
    $key = $theme->get_css_cache_key() . $subtheme;
